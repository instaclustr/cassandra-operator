...
func createOrUpdateCassandraRackConfig(rctx *reconciliationRequestContext, rack string) (*corev1.Volume, error) {

	configMap := &corev1.ConfigMap{ObjectMeta: DataCenterResourceMetadata(rctx.cdc, "operator-rack-config")}
	...
	configVolume := &corev1.Volume{
		Name:         "operator-rack-config-volume",
		VolumeSource: corev1.VolumeSource{ConfigMap: volumeSource},
	}
...
func addCassandraGossipingPropertyFileSnitchProperties(cdc *cassandraoperatorv1alpha1.CassandraDataCenter, rack string, addFileFn func(path string, data string)) {
